...
async function renderCalendar() {
    ...
    try {
        ...
        // Calculate Streak (Async)
        calculateStreakForTrainer(selectedStudentForHistory);
...
async function calculateStreakForTrainer(studentId) {
    if (!studentId) return;
    ...
    try {
        ...
        if (elCurr) elCurr.innerText = currentStreak;
        if (elMax) elMax.innerText = maxStreak;

    } catch (e) {
        console.error("Trainer streak calc error", e);
    }
}
